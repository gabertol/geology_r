...
- Pesquisador em projeto de cooperação UFRGS-University of Aberdeen
  - Pesquisador Pós-Doutorado "O efeito do padrão de ventos na composição detrítica de arenitos eólicos: Um exemplo na Formação Pirambóia " no PRH-14 (UFRGS)
...
- Pacotes de estatistica em geologia
  ...
  - Tidypaleo e StratigrapheR (Pacote para dados estratigráficos)
...
- Qual a distribuição dos elementos químicos maiores em diferentes ambiente geotectônicos? 
...
read.csv(file="geoquim_granite.csv",
                     sep=";") %>% 
  dplyr::select(Group,Or,Ab,An,TASMiddlemostPlut) %>%
  drop_na() %>% 
...
```{r BBBBB, eval=FALSE}
ages %>% 
  dplyr::rename(Hemi_long="e_w",
         Hemi_lat="n_s",
         Peate="parana_classification_peate_et_al_1992") %>%
  mutate(class=case_when(Peate==c("Aigua (Palmas equivalent)","India Muerta (Palmas equivalente)","Las Averías (Palmas equivalente)","Lascano (Palmas equivalente)","Lascano (Palmas equivalente?)","Palmas","Palmas?")~"Palmas",
                        Peate==c("Santa Lucía (Gramado equivalent)","Santa Lucía (Gramado or Ribeira equivalent)","Treinta y Tres (Gramado equivalent)","Gramado","Gramado?")~"Gramado",
                        Peate==c("Esmeralda","Esmeralda?")~"Esmeralda",
                        Peate==c("Pitanga","Pitanga??")~"Pitanga",
                        Peate==c("Urubici","Urubici?")~"Urubici",
                        Peate==c("Chapecó")~"Chapecó",
                        Peate==c("Ribeira","Ribeira?")~"Ribeira",
                        TRUE~"Uncertain"
  )) %>% 
  ...
  dplyr::select(class,recalculated_age_ma) %>% 
  mapview(zcol="class")
```
]
...
ages %>% 
  ...
  unite(long_DMS,longitude,Hemi_long, sep="") %>% 
  unite(lat_DMS,latitude,Hemi_lat, sep="") %>%
  mutate(age=as.double(recalculated_age_ma),
...
library(raster)
library(stars)
map4<-stars::read_stars("Early_K_paleomap.tif")
```
---
...
---
#<b> Bibliotecas desenvolvidas na UFRGS</b>

## stratools (Dados estratigráficos)
- Biblioteca em desenvolvimento para perfis estratigraficos e correlações estratigráficas

https://github.com/gabertol/stratools

```{r,echo=FALSE,fig.align='center',out.width="60%"}
 knitr::include_graphics("stratigraphy.jpg")
```

---

## ztR (Quimica mineral)
- Biblioteca para converter geoquimica mineral de %wt para APFU e end-members (em desenvolvimento)
- Baseado no livro seminal DHZ com 3 benchmarks (granada,turmalina)
https://github.com/gabertol/ztR
```{r,echo=FALSE,fig.align='center',out.width="50%"}
 knitr::include_graphics("ztr.jpg")
```
## ggarnet (Overlays de geoquimica para ggplot)
- Biblioteca com overlay de classificação de diagramas ternários (incluindo atualmente granadas e turmalina) 
- Gabriel Bertolini e João Miguel Maraschin
https://github.com/gabertol/ggarnet


---
# <b>Mensagem para levar pro berço</b>
...
- Pacotes/Biblioteca resolvem os mais variados problemas 
